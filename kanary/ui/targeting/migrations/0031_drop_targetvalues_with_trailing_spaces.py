...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Drop target values with trailing spaces."
        TV = orm['targeting.TargetValue']

        (TV.objects.filter(value__contains='\x1d ') |
                TV.objects.filter(value__contains=' \x1d') |
                TV.objects.filter(value__startswith=' ') |
                TV.objects.filter(value__endswith=' ')).delete()
