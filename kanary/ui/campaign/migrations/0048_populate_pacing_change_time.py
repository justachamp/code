...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Set initial spent for paced strategy
        for strategy in orm['campaign.Strategy'].objects.filter(is_distributed_evenly=True):
            strategy.last_pacing_change_UTC = std_datetime.utcnow()
            strategy.total_spent_at_pacing_start = strategy.budget_spent + strategy.budget_spent_commission
            strategy.daily_spent_at_pacing_start = strategy.budget_daily_spent
            strategy.save()

    def backwards(self, orm):
        # Restore default null values
        for strategy in orm['campaign.Strategy'].objects.filter(is_distributed_evenly=True):
            strategy.last_pacing_change_UTC = None
            strategy.total_spent_at_pacing_start = None
            strategy.daily_spent_at_pacing_start = None
            strategy.save()
