/**
* Datepicker with corrected getDaysInMonth method based on moment.js
*
* @version: 1.0.1
...
!function ($) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        var DRPTemplate =
            '<div class="datepicker dropdown-menu">' +
                '<div class="calendars clearfix">' +
                    '<div class="calendar one"></div>' +
                    '<div class="calendar left"></div>' +
                    '<div class="calendar right"></div>' +
                    '<div class="ranges -t-ranges"></div>' +
                '</div>' +
                '<div class="ranges-container">' +
                    '<h4><span>' + this.locale.customRangeLabel + '</span></h4>' +
                    '<div class="ranges-inputs">' +
                        '<div class="daterangepicker_start_input">' +
                            '<label for="daterangepicker_start">' + this.locale.fromLabel + '</label>' +
                            '<input class="input input-block-level" type="text" name="daterangepicker_start" value="" disabled="disabled" />' +
                        '</div>' +
                        '<div class="daterangepicker_end_input">' +
                            '<label for="daterangepicker_end">' + this.locale.toLabel + '</label>' +
                            '<input class="input input-block-level" type="text" name="daterangepicker_end" value="" disabled="disabled" />' +
                        '</div>' +
                        '<button class="btn btn-yellow applyBtn span4 datepicker-apply" disabled="disabled">' + this.locale.applyLabel + '</button>&nbsp;' +
                    '</div>' +
                '</div>' +
            '</div>';
        ...
        if(this.element.data()){
            if (this.element.data('range'))
                this.range = this.element.data('range');

            if (this.element.data('picker-class'))
                this.container.addClass(this.element.data('picker-class'));
        }
        ...
        this.container.find('.ranges-container').on('click', 'button.applyBtn', $.proxy(this.clickApply, this));
    ...
    DateRangePicker.prototype = {
        ...
        notify: function () {
            ...
            if (this.element.is('input') && this.range) {
                this.element.val(this.cleared ? '' : this.startDate.format(this.format) + this.separator + this.endDate.format(this.format));
            } else if (this.range){
                this.element.find('input').val(this.cleared ? '' : this.startDate.format(this.format) + this.separator + this.endDate.format(this.format));
            }
        ...
        clickPrev: function (e) {
            ...
            if (cal.hasClass('left')) {
                ...
                this.oneCalendar.month.addMonths(-1);
            }
            this.updateCalendars();
        },
        ...
        clickDate: function (e) {
            ...
            if (cal.hasClass('left')) {
                ...
                date = this.oneCalendar.calendar[row][col];
                this.element.trigger('clicked',{
                    dir: 'one',
                    picker: this
                });
            ...
            if(this.range)
            {
                ...
                this.date = date;
                this.changed = true;
                this.oneCalendar.month.set({ month: this.date.getMonth(), year: this.date.getFullYear() });

                if (this.element.is('input'))
                    this.element.val(this.oneCalendar.calendar[row][col].format(this.format));
                else
                    this.element.find('input').val(this.oneCalendar.calendar[row][col].format(this.format));
            }
            this.updateCalendars();
        },
        ...
        updateCalendars: function () {
            if(this.range){
                ...
                this.oneCalendar.calendar = this.buildCalendar(this.oneCalendar.month.getMonth(), this.oneCalendar.month.getFullYear());
                this.container.find('.calendar.one').html(this.renderCalendar(this.oneCalendar.calendar, this.date, this.minDate, this.maxDate));
                this.element.trigger('updated',this);
            }
        },
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            {
                html += '<th class="prev available calendar-header"><i class="arrow-black-left"></i></th>';
            }
            else
            {
                 html += '<th></th>';
            }
            html += '<th colspan="5" class="month-name calendar-header" style="width: auto">' + this.locale.monthNames[calendar[1][1].getMonth()] + calendar[1][1].format(' {yyyy}') + '</th>';
            if (!maxDate || maxDate > calendar[1][1])
            {
                html += '<th class="next available calendar-header"><i class="arrow-black-right"></i></th>';
            }
            else
            {
                 html += '<th></th>';
            ...
            $.each(this.locale.daysOfWeek, function (index, dayOfWeek) {
                html += '<th class="week-days">' + dayOfWeek + '</th>';
            });
        ...
        getDaysInMonth: function (y, m) {
            /**
            Days in a given month of a given year. Month numbers are starting from 1.
            */
