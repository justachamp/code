...
# Names we migrate to. We cannot take those values from Strategy because they
# may have already changed in code and future migrations should handle that.
SOCIAL = 'social'
SITE = 'site'
MOBILE = 'app'
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Update strategy types names."
        migration_map = {
            'w': SITE,
            # Strategy.type used to be initialized with wrong value:
            'Web': SITE,
            's': SOCIAL,
            'm': MOBILE,
        }
        # Accept already valid data:
        for name in (SOCIAL, SITE, MOBILE):
            migration_map[name] = name

        for strategy in orm.Strategy.objects.all():
            old_type = strategy.type
            try:
                strategy.type = migration_map[old_type]
                strategy.save()
            except KeyError:
                raise Exception('Unknown strategy type: ' + old_type)

    def backwards(self, orm):
        "Put old strategy types names in strategies."
        reverse_map = {
            SITE: 'w',
            SOCIAL: 's',
            MOBILE: 'm',
        ...
        for strategy in orm.Strategy.objects.all():
            prev_type = strategy.type
            try:
                strategy.type = reverse_map[prev_type]
                strategy.save()
            except KeyError:
                raise Exception('Unknown strategy type: ' + prev_type)
