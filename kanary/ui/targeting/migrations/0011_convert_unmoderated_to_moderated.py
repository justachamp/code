...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        Take unmoderated TargetValues (representant == self) and make
        them moderated (representant == None).
        '''
        TargetValue = orm['targeting.TargetValue']
        tvs = TargetValue.objects.exclude(category='publisher')
        # Get only those TV's that point to themselves (unmoderated):
        tvs_self = tvs.filter(representant__pk=models.F('pk'))
        tvs_self.update(representant=None)

    def backwards(self, orm):
        ''' There's nothing to do. We won't be making TargetValues invalid. '''
