...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm['campaign.Campaign'].objects.filter(is_active=True).update(is_paused=False)
        orm['campaign.Campaign'].objects.filter(is_active=False).update(is_paused=True)
        orm['campaign.Strategy'].objects.filter(is_active=True).update(is_paused=False)
        orm['campaign.Strategy'].objects.filter(is_active=False).update(is_paused=True)

    def backwards(self, orm):
        "Write your backwards methods here."
        orm['campaign.Campaign'].objects.filter(is_paused=False).update(is_active=True)
        orm['campaign.Campaign'].objects.filter(is_paused=True).update(is_active=False)
        orm['campaign.Strategy'].objects.filter(is_paused=False).update(is_active=True)
        orm['campaign.Strategy'].objects.filter(is_paused=True).update(is_active=False)
