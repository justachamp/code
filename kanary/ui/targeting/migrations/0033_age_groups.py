...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for auto_age_group in orm['targeting.TargetValue'].objects.filter(category='age_group').all():
            age_group = orm['targeting.TargetValue'].objects.filter(
                exchange_id=auto_age_group.exchange_id,
                category='age_groups',
                value=auto_age_group.value
            ).first()
            if not age_group:
                # if there's no corresponding target value,. simply move back what migration 32 did.
                auto_age_group.category = 'age_groups'
                auto_age_group.save()
            else:
                # If there is unfortunately, we need to switch reports back:
                for report_model in [orm['report.ReportStrategyAge'], orm['report.ReportCampaignAge']]:
                    for report_entry in report_model.objects.filter(dimension_id=auto_age_group.pk):
                        age_group_entry = report_model.objects.filter(
                            dimension_id=age_group.pk,
                            time=report_entry.time,
                            related_id=report_entry.related_id
                        ).first()
                        # luckily, we haven't gathered anything for the old group
                        if not age_group_entry:
                            report_entry.dimension_id = age_group.pk
                            report_entry.save()
                        # unfortunately, we need to sum data
                        else:
                            age_group_entry.bid = models.F('bid') + report_entry.bid
                            age_group_entry.bid_value = models.F('bid_value') + report_entry.bid_value
                            age_group_entry.winbid = models.F('winbid') + report_entry.winbid
                            age_group_entry.winbid_value = models.F('winbid_value') + report_entry.winbid_value
                            age_group_entry.data_cost_value = models.F('data_cost_value') + report_entry.data_cost_value
                            age_group_entry.commission_value = models.F('commission_value') + report_entry.commission_value
                            age_group_entry.imp = models.F('imp') + report_entry.imp
                            age_group_entry.clk = models.F('clk') + report_entry.clk
                            age_group_entry.conv = models.F('conv') + report_entry.conv
                            age_group_entry.conv_value = models.F('conv_value') + report_entry.conv_value
                            age_group_entry.save()
                            report_entry.delete()
                auto_age_group.delete()

        # rename to age_group back
    ...
    complete_apps = ['report', 'targeting']
    symmetrical = True
