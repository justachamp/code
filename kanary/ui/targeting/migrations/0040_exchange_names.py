...
exchange_mapping = {'1': 'nexage', '2': 'appnexus', '3': 'smaato'}
...
case = "    WHEN exchange = %r THEN %r"
up_cases = '\n'.join(case % (k, v) for k, v in exchange_mapping.items())
down_cases = '\n'.join(case % (v, k) for k, v in exchange_mapping.items())
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Change exchange ids stringified by the prev. migration to exchange names
        in TargetValue.exchange and ContentCategoryValue.exchange.
        """
        for table in [model._meta.db_table for model in [orm.TargetValue, orm.ContentCategoryValue]]:
            sql = sql_template.format(cases=up_cases, table=table)
            print sql
            db.execute(sql)

    def backwards(self, orm):
        """
        Change exchange exchange names to ids in strings in TargetValue.exchange and ContentCategoryValue.exchange.
        """
        for table in [model._meta.db_table for model in [orm.TargetValue, orm.ContentCategoryValue]]:
            sql = sql_template.format(cases=down_cases, table=table)
            print sql
            db.execute(sql)
